...
async def cmd_status(message: types.Message):
    ...
    # Дополнительно получаем количество боёв и урон из таблицы chat_authority
...
async def cmd_top(message: types.Message):
    ...
    await show_top(chat_id, message, page)
...
async def show_top(chat_id: int, message: types.Message, page: int = 1, order: str = "authority"):
    ...
    # Кнопки пагинации и переключения категорий
    kb = InlineKeyboardMarkup(row_width=3)
    nav = []
    if page > 1:
        nav.append(InlineKeyboardButton("⬅️", callback_data=f"top_page_{order}_{page-1}"))
